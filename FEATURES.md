...
### Can Depo Helper (`CanDepoHelper.js`)
...
### Candy Store Helper (`CandyStoreHelper.js`)
...
### Recycling Bin Helper (`RecyclingBinHelper.js`)
...
### Market Helper (`MarketHelper.js`)
Enhances the main user Market.
...
### Tattoo Parlor Helper (`TattooParlorHelper.js`)
...
### University Helper (`UniversityHelper.js`)
...
### Wellness Clinic Helper (`WellnessClinicHelper.js`)
Enhances the Wellness Clinic by displaying an interactive cumulative hospital fee tracker and allows you to set specific levels as bank goals directly into the UI.

### Hospital Helper (`HospitalHelper.js`)
Assists at the Hospital by tracking the cooldown timers between heal actions.

### Skills Helper (`SkillsHelper.js`)
Enhances the Skills area by adding a Max Buy button for skill sets and allowing you to reposition the Skill Shop link.

### Backpack Helper (`BackpackHelper.js`)
...
### Active List Helper (`ActiveListHelper.js`)
...
### Drinks & Mixing System (`LiquorStoreHelper.js`, `MixerHelper.js`)
A suite of tools centered around drinking and stat enhancement:
- Information on in-game drinks and their potential effects via tooltips in your backpack/living area.
...
### Food Menu Management (`FoodHelper.js`)
Enhances the food inventory screen (and Living Area Food tab).
- Add a "Mark as Crap" button to mark selected foods as "crap". Unchecked foods will be removed from the list.
- Add a "Select Crap" button to automatically select all foods you have marked as crap, making it easy to throw them away.

### Food Bank Helper (`FoodBankHelper.js`)
Improves the layout of the food bank making it easier to select, freeze, and unfreeze foods.

### Soup Kitchen Helper (`SoupKitchenHelper.js`)
...
### Living Area Helper (`LivingAreaHelper.js`)
...
### Northern Fence Helper (`NorthernFenceHelper.js`)
Assists in navigating the Northern Fence.
...
### Fort Slugworth Helper (`FortSlugworthHelper.js`)
...
### Kurtz Camp Helper (`KurtzCampHelper.js`)
...
### Bernards Basement Helper (`BernardsBasementHelper.js`)
...
### Mine Pattern Helper (`MinePatternHelper.js`)
...
### Weapons Helper (`WeaponsHelper.js`)
...
### Gang Hitlist Helper (`GangHitlistHelper.js`)
...
### Gang Board Staff Helper (`GangBoardStaffHelper.js`)
...
### Battle Helpers (`BattleHelper.js`, `BattleLogHelper.js`)
- **Battle Helper:** Adds a button to view a line graph of the fight inside a floating panel on the results page.
- **Battle Log Helper:** Scans your battle log to track fought hobos.
...
### Message Board Helper (`MessageBoardHelper.js`)
...
### Display Helper (`DisplayHelper.js`)
A core helper that runs display improvement related functions globally.
...
### Settings Helper (`SettingsHelper.js`)
...
### Cloud Sync Helper (`SyncHelper.js`)
...
### Lockout Helper (`LockoutHelper.js`)
...
### City Statistics Helper (`CityStatisticsHelper.js`)
